...
def random_split(data_list, val_proportion=0.1, test_proportion=0.1, seed=0):
    ...
    split = ['test'] * num_test_mols + ['val'] * \
        num_val_mols + ['train'] * num_train_mols
    ...
    save_pickle(
        train_data, '/home//MoleculeDiffusion/data/geom/rdkit_folder/train_data.pickle')
    save_pickle(
        val_data, '/home/vignac/MoleculeDiffusion/data/geom/rdkit_folder/val_data.pickle')
    save_pickle(
        test_data, '/home//MoleculeDiffusion/data/geom/rdkit_folder/test_data.pickle')
    return
    ...
    for i, drugs_1k in enumerate(unpacker):
        ...
        for smiles, all_info in drugs_1k.items():
            ...
            for id in lowest_energies:
                ...
                coords = np.array(conformer['xyz']).astype(
                    float)        # n x 4
                if args.remove_h:
                    mask = coords[:, 0] != 1.0
                    coords = coords[mask]
