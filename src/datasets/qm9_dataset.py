...
class QM9Dataset(InMemoryDataset):
    ...
    def __init__(self, split, root, remove_h: bool, transform=None, pre_transform=None, pre_filter=None):
        ...
        if remove_h:
            self.atom_encoder = {k: v - 1 for k,
                                 v in self.atom_encoder.items() if k != 'H'}
        ...
        self.statistics = Statistics(num_nodes=load_pickle(self.processed_paths[1]),
                                     atom_types=torch.from_numpy(
                                         np.load(self.processed_paths[2])).float(),
                                     bond_types=torch.from_numpy(
                                         np.load(self.processed_paths[3])).float(),
                                     charge_types=torch.from_numpy(
                                         np.load(self.processed_paths[4])).float(),
                                     valencies=load_pickle(
                                         self.processed_paths[5]),
                                     bond_lengths=load_pickle(
                                         self.processed_paths[6]),
    ...
    def download(self):
        ...
        train, val, test = np.split(dataset.sample(
            frac=1, random_state=42), [n_train, n_val + n_train])
...
class QM9DataModule(AbstractDataModule):
    def __init__(self, cfg):
        ...
        train_dataset = QM9Dataset(
            split='train', root=root_path, remove_h=cfg.dataset.remove_h)
        val_dataset = QM9Dataset(
            split='val', root=root_path, remove_h=cfg.dataset.remove_h)
        test_dataset = QM9Dataset(
            split='test', root=root_path, remove_h=cfg.dataset.remove_h)
        self.statistics = {'train': train_dataset.statistics, 'val': val_dataset.statistics,
                           'test': test_dataset.statistics}
        self.remove_h = cfg.dataset.remove_h
        super().__init__(cfg, train_dataset=train_dataset,
...
class QM9infos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg):
        ...
        self.input_dims = PlaceHolder(
            X=self.num_atom_types, charges=3, E=5, y=1, pos=3)
