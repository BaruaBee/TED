...
class NoiseModel:
    def __init__(self, cfg):
        ...
        self._gamma = torch.log(-torch.special.expm1(2 *
                                log_alpha_bar)) - 2 * log_alpha_bar
    ...
    def get_Qt(self, t_int):
        ...
        q_x = bx * P.X + (1 - bx) * torch.eye(self.X_classes,
                                              **kwargs).unsqueeze(0)

        bc = self.get_beta(t_int=t_int, key='c').unsqueeze(1)
        q_c = bc * P.charges + \
            (1 - bc) * torch.eye(self.charges_classes, **kwargs).unsqueeze(0)

        be = self.get_beta(t_int=t_int, key='e').unsqueeze(1)
        q_e = be * P.E + (1 - be) * torch.eye(self.E_classes,
                                              **kwargs).unsqueeze(0)

        by = self.get_beta(t_int=t_int, key='y').unsqueeze(1)
        q_y = by * P.y + (1 - by) * torch.eye(self.y_classes,
                                              **kwargs).unsqueeze(0)
    ...
    def get_Qt_bar(self, t_int):
        ...
        # [X, charges, E, y, pos]
        dev = t_int.device
        q_x = a_x * torch.eye(self.X_classes,
                              device=dev).unsqueeze(0) + (1 - a_x) * P.X
        q_c = a_c * torch.eye(self.charges_classes,
                              device=dev).unsqueeze(0) + (1 - a_c) * P.charges
        q_e = a_e * torch.eye(self.E_classes,
                              device=dev).unsqueeze(0) + (1 - a_e) * P.E
        q_y = a_y * torch.eye(self.y_classes,
                              device=dev).unsqueeze(0) + (1 - a_y) * P.y
    ...
    def get_alpha_pos_ts(self, t_int, s_int):
        log_a_bar = self._log_alpha_bar[...,
                                        self.inverse_mapping['p']].to(t_int.device)
    ...
    def apply_noise(self, dense_data, current_step=None):
        """ Sample noise and apply it to the data. """
        # print(type(dense_data))
        ...
        if current_step is None:
            ...
            t_int = torch.randint(
                1, current_step + 1, size=(dense_data.X.size(0), 1), device=device)
            t_float = t_int.float() / current_step
        ...
        assert (dense_data.X.shape == X_t.shape) and (
            dense_data.E.shape == E_t.shape)

        noise_pos = torch.randn(dense_data.pos.shape,
                                device=dense_data.pos.device)
        noise_pos_masked = noise_pos * dense_data.node_mask.unsqueeze(-1)
        noise_pos_masked = utils.remove_mean_with_mask(
            noise_pos_masked, dense_data.node_mask)
        ...
        z_t = utils.PlaceHolder(X=X_t, charges=charges_t, E=E_t, y=dense_data.y, pos=pos_t, t_int=t_int,
                                t=t_float, node_mask=dense_data.node_mask, batch=dense_data.batch).mask()
        return z_t
    ...
    def sample_limit_dist(self, node_mask):
        ...
        e_limit = self.E_marginals[None, None,
                                   None, :].expand(bs, n_max, n_max, -1)
        charges_limit = self.charges_marginals.expand(bs, n_max, -1)

        U_X = x_limit.flatten(
            end_dim=-2).multinomial(1).reshape(bs, n_max).to(node_mask.device)
        U_c = charges_limit.flatten(
            end_dim=-2).multinomial(1).reshape(bs, n_max).to(node_mask.device)
        U_E = e_limit.flatten(
            end_dim=-2).multinomial(1).reshape(bs, n_max, n_max).to(node_mask.device)
        ...
        pos = torch.randn(
            node_mask.shape[0], node_mask.shape[1], 3, device=node_mask.device)
    ...
    def sample_zs_from_zt_and_pred(self, z_t, pred, s_int):
        ...
        z_t_prefactor = (self.get_alpha_pos_ts(
            t_int=t_int, s_int=s_int) * sigma_sq_ratio).unsqueeze(-1)
        x_prefactor = self.get_x_pos_prefactor(
            s_int=s_int, t_int=t_int).unsqueeze(-1)

        mu = z_t_prefactor * z_t.pos + x_prefactor * pred.pos             # bs, n, 3

        sampled_pos = torch.randn(
            z_t.pos.shape, device=z_t.pos.device) * node_mask.unsqueeze(-1)
        ...
        prefactor2 = self.get_sigma2_bar(
            t_int=s_int, key='p') * self.get_alpha_pos_ts_sq(t_int=t_int, s_int=s_int)
        ...
        # bs, n, d0, d_t-1
        weighted_c = pred_charges.unsqueeze(-1) * p_s_and_t_given_0_c
        unnormalized_prob_c = weighted_c.sum(
            dim=2)                     # bs, n, d_t-1
        unnormalized_prob_c[torch.sum(unnormalized_prob_c, dim=-1) == 0] = 1e-5
        prob_c = unnormalized_prob_c / \
            torch.sum(unnormalized_prob_c, dim=-1,
                      keepdim=True)  # bs, n, d_t-1
        ...
        charges_s = F.one_hot(
            sampled_s.charges, num_classes=self.charges_classes).float()
...
class DiscreteUniformTransition(NoiseModel):
    def __init__(self, cfg, output_dims):
        ...
        self.charges_marginals = torch.ones(
            self.charges_classes) / self.charges_classes
        self.E_marginals = torch.ones(self.E_classes) / self.E_classes
        self.y_marginals = torch.ones(self.y_classes) / self.y_classes
        self.Px = torch.ones(
            1, self.X_classes, self.X_classes) / self.X_classes
        self.Pcharges = torch.ones(
            1, self.charges_classes, self.charges_classes) / self.charges_classes
        self.Pe = torch.ones(
            1, self.E_classes, self.E_classes) / self.E_classes
        self.Pe = torch.ones(
            1, self.y_classes, self.y_classes) / self.y_classes
...
class MarginalUniformTransition(NoiseModel):
    def __init__(self, cfg, x_marginals, e_marginals, charges_marginals, y_classes):
        ...
        self.Px = x_marginals.unsqueeze(0).expand(
            self.X_classes, -1).unsqueeze(0)
        self.Pe = e_marginals.unsqueeze(0).expand(
            self.E_classes, -1).unsqueeze(0)
        self.Pcharges = charges_marginals.unsqueeze(
            0).expand(self.charges_classes, -1).unsqueeze(0)
        self.Py = torch.ones(
            1, self.y_classes, self.y_classes) / self.y_classes
