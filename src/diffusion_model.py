...
from src.eval_rdkit_pkl import evaluate
...
class FullDenoisingDiffusion(pl.LightningModule):
    ...
    def __init__(self, cfg, dataset_infos, train_smiles):
        ...
        self.eval_type = eval_type = 'both'
        self.sub_geometry = 'True'
        self.root_path = "/home/barry/download/prog/data/"
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.name == "debug" or (self.val_counter % self.cfg.general.sample_every_val == 0 and
            ...
            samples,rdkit_samples = self.sample_n_graphs(samples_to_generate=math.ceil(gen.samples_to_generate / max(gen.gpus, 1)),
            ...
            file_path = os.path.join(current_path, f'graphs/epoch{self.current_epoch}/data.pkl')
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            with open(file_path, 'wb') as file:
                pickle.dump(rdkit_samples, file)
            dataset_name = self.dataset_infos.name
            results = evaluate(file_path, dataset_name, eval_type=self.eval_type, sub_geometry=self.sub_geometry, root_path=self.root_path)
            print(results)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        samples,rdkit_samples = self.sample_n_graphs(samples_to_generate=self.cfg.general.final_model_samples_to_generate,
        ...
        current_path = os.getcwd()
        pklname = f'generated_samples.pkl'
        file_path = os.path.join(current_path, pklname)
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, 'wb') as file:
                pickle.dump(rdkit_samples, file)
        dataset_name = 'qm9'
    ...
    def sample_n_graphs(self, samples_to_generate: int, chains_to_save: int, samples_to_save: int, test: bool):
        ...
        samples.extend(self.sample_batch(n_nodes=current_n_list, test=test))
        rdkit_mol_list = []
        rdkit_mol_list = [molecule.rdkit_mol for molecule in samples]

        return samples, rdkit_mol_list
