...
def to_dense(data, dataset_info, device=None):
    ...
    data = PlaceHolder(X=X, charges=charges, pos=pos, E=E, y=y, node_mask=node_mask,batch=batch)
    return data.mask()


class PlaceHolder:
    def __init__(self, pos, X, charges, E, y, t_int=None, t=None, node_mask=None, edge_index=None,batch=None):
        self.pos = pos
        self.X = X
    ...
    def copy(self):
        return PlaceHolder(X=self.X, charges=self.charges, E=self.E, y=self.y, pos=self.pos, t_int=self.t_int, t=self.t,
                           node_mask=self.node_mask, edge_index=self.edge_index)
